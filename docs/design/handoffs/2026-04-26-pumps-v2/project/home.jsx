...
const COPY = {
  fr: {
    ...
    today: "Aujourd'hui",
    morning: 'Matin', evening: 'Soir', noon: 'Midi',
  ...
  en: {
    ...
    today: 'Today',
    morning: 'Morning', evening: 'Evening', noon: 'Noon',
...
const Icon = {
  ...
  clock: (size = 14, color = 'currentColor') => (
    <svg width={size} height={size} viewBox="0 0 14 14" fill="none" stroke={color} strokeWidth="1.5">
  ...
  wifi: (size = 12, color = 'currentColor') => (
    <svg width={size} height={size} viewBox="0 0 12 12" fill="none" stroke={color} strokeWidth="1.3" strokeLinecap="round">
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: toneStyle.dot, marginTop: 7, flexShrink: 0,
        boxShadow: `0 0 0 4px color-mix(in oklch, ${toneStyle.dot} 18%, transparent)`,
      }} />
      <div style={{ flex: 1, minWidth: 0 }}>
        <div className="k-display" style={{
          fontSize: 22, lineHeight: 1.15, color: toneStyle.ink, fontWeight: 500,
        }}>{title}</div>
        <div style={{
          fontSize: 13, color: 'var(--k-ink-2)', marginTop: 4,
  ...
  return (
    <button
      ...
      style={{
        ...
        background: bg, color: fg, textAlign: 'left',
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{
          fontSize: 12, opacity: isMaint ? 0.85 : 0.7, marginTop: 2,
    ...
    <div className="k-section">
      ...
      <div style={{
        ...
        {slots.map((s, i) => {
          ...
          if (s.state === 'done') {
            dotBg = 'var(--k-ok)'; ring = 'var(--k-ok-soft)'; ink = 'oklch(35% 0.08 155)'; text = t.done;
          } else if (s.state === 'pending') {
            dotBg = accentColor; ring = 'color-mix(in oklch, var(--k-maint) 12%, transparent)'; ink = 'var(--k-maint-ink)'; text = t.pending;
          } else if (s.state === 'overdue') {
            dotBg = 'var(--k-amber)'; ring = 'var(--k-amber-soft)'; ink = 'var(--k-amber-ink)'; text = t.overdue;
          } else {
            dotBg = 'var(--k-miss)'; ring = 'var(--k-miss-soft)'; ink = 'var(--k-ink-3)'; text = t.missed;
          }
          return (
            <div key={i} style={{
              ...
              <div style={{
                fontSize: 11, color: 'var(--k-ink-3)',
                textTransform: 'uppercase', letterSpacing: '0.08em',
                fontWeight: 500,
              }}>{s.label}</div>
              <div className="k-mono" style={{
                fontSize: 16, fontWeight: 500, color: 'var(--k-ink)',
                fontVariantNumeric: 'tabular-nums',
              }}>{s.time}</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 'auto' }}>
                <span style={{
                  width: 6, height: 6, borderRadius: 99, background: dotBg,
...
function Activity({ t, role }) {
  const items = [
    {
      kind: 'maint', who: t.coparent, time: '8:02', ago: fmt(t.hAgo, { n: 11 }),
      label: t.pillFond,
    },
    {
      kind: 'maint', who: t.daycare, time: '12:15', ago: fmt(t.hAgo, { n: 7 }),
      label: t.pillFond, syncNote: '12:18',
    },
    {
      kind: 'rescue', who: t.you, time: 'Lun · 14:40', ago: '3 j',
      label: t.pillSecours, cause: t.causeEffort,
    },
  ];
  ...
  return (
    <div className="k-section">
      <SectionHeader label={t.activity} action={
        <a style={{
          fontSize: 12, color: 'var(--k-maint)', textDecoration: 'none',
          fontWeight: 500, display: 'flex', alignItems: 'center', gap: 4,
        }}>{t.history} {Icon.arrow(11, 'var(--k-maint)')}</a>
      } />
            ...
            <div key={i} style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, flexWrap: 'wrap' }}>
                  <span style={{ fontSize: 13.5, color: 'var(--k-ink)', fontWeight: 500 }}>{it.label}</span>
                  {it.cause && (
                    <span style={{
                      fontSize: 11, padding: '1px 6px',
                      background: 'var(--k-rescue-soft)', color: 'var(--k-rescue-ink)',
                ...
                <div style={{ fontSize: 12, color: 'var(--k-ink-3)', marginTop: 2 }}>
                  ...
                  {it.syncNote && <span style={{ color: 'var(--k-ink-4)' }}> · sync {it.syncNote}</span>}
                </div>
              </div>
              <div className="k-mono" style={{
                fontSize: 12, color: 'var(--k-ink-3)',
                fontVariantNumeric: 'tabular-nums',
              }}>{it.time}</div>
            </div>
          );
...
function Caregivers({ t, role }) {
  const people = [
    ...
    { name: t.coparent, role: t.role_contributor, online: true, color: 'oklch(60% 0.08 290)', initial: 'C' },
    { name: t.daycare, role: t.role_restricted, online: false, color: 'oklch(60% 0.08 75)', initial: 'G', syncPending: true },
    { name: t.grandparent, role: t.role_contributor, online: true, color: 'oklch(60% 0.07 155)', initial: 'P' },
  ];
  if (role === 'restricted') return null;
      ...
      <div style={{ display: 'flex', gap: 16, overflowX: 'auto' }}>
        {people.map((p, i) => (
          <div key={i} style={{
            ...
            <div style={{
              fontSize: 11, color: 'var(--k-ink-2)', fontWeight: 500,
              textAlign: 'center', lineHeight: 1.2,
            }}>{p.name}</div>
            <div style={{
              fontSize: 10, color: 'var(--k-ink-4)', textTransform: 'uppercase',
              letterSpacing: '0.06em',
            }}>{p.syncPending ? t.syncPending : p.role}</div>
          </div>
        ))}
      </div>
    </div>
...
function Inventory({ t, role, lang }) {
  ...
  const inhalers = [
    {
      name: 'Fluticasone 50µg', sub: lang === 'fr' ? 'Pompe de fond · matin & soir' : 'Maintenance · morning & evening',
      kind: 'maint', doses: 92, total: 120, expiry: '2026-09-14',
    },
    {
      name: 'Salbutamol 100µg', sub: lang === 'fr' ? 'Pompe de secours · maison' : 'Rescue · home',
      kind: 'rescue', doses: 18, total: 200, expiry: '2026-05-22', low: true,
    },
    {
      name: 'Salbutamol 100µg', sub: lang === 'fr' ? 'Pompe de secours · garderie' : 'Rescue · daycare',
      kind: 'rescue', doses: 144, total: 200, expiry: '2027-02-10',
    },
  ];

  return (
    <div className="k-section">
      <SectionHeader label={t.inventory} action={
        <span style={{ fontSize: 12, color: 'var(--k-maint)', fontWeight: 500 }}>+</span>
      } />
            ...
            <div key={i} style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 8 }}>
                  <span style={{ fontSize: 14, color: 'var(--k-ink)', fontWeight: 500 }}>{inh.name}</span>
                  <span className="k-mono" style={{
                    fontSize: 12, color: inh.low ? 'var(--k-amber-ink)' : 'var(--k-ink-2)',
                    fontVariantNumeric: 'tabular-nums', fontWeight: 500,
                    whiteSpace: 'nowrap',
                  }}>{fmt(t.dosesLeftLong, { n: inh.doses })}</span>
                </div>
                <div style={{ fontSize: 11.5, color: 'var(--k-ink-3)', marginTop: 2 }}>{inh.sub}</div>
                <div style={{
                  marginTop: 8, height: 5, background: 'var(--k-line)',
                  borderRadius: 99, overflow: 'hidden', position: 'relative',
                }}>
                  <div style={{
                    width: `${stockPct}%`, height: '100%',
                    background: inh.low ? 'var(--k-amber)' : (isMaint ? 'var(--k-maint)' : 'var(--k-rescue)'),
                    borderRadius: 99,
                  }} />
                </div>
                <div style={{
                  display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                  marginTop: 7, gap: 8, flexWrap: 'wrap',
                }}>
                  <span style={{
                    fontSize: 11, color: expired ? 'var(--k-rescue-ink)' : (expiringSoon ? 'var(--k-amber-ink)' : 'var(--k-ink-3)'),
                    display: 'inline-flex', alignItems: 'center', gap: 5,
                  }}>
                    {Icon.clock(11, expired ? 'var(--k-rescue-ink)' : (expiringSoon ? 'var(--k-amber-ink)' : 'var(--k-ink-3)'))}
                    {expired
                      ? t.expired
                      : expiringSoon
                  ...
                  {showWarn && inh.low && (
                    <span style={{
                      fontSize: 11, color: 'var(--k-amber-ink)',
...
function BottomActionBar({ t, accent, role, dark }) {
  ...
  const Btn = ({ kind }) => {
    ...
    return (
      <button style={{
        ...
        boxShadow: isMaint ? '0 6px 18px color-mix(in oklch, ' + accentColor + ' 35%, transparent)' : '0 1px 0 rgba(0,0,0,0.02)',
        minHeight: 76,
      }}>
        <div style={{
        ...
        <span className="k-display" style={{ fontSize: 13, fontWeight: 600, letterSpacing: '0.01em' }}>
  ...
  return (
    <div style={{
      ...
      <div style={{
        fontSize: 10, color: 'var(--k-ink-3)', textTransform: 'uppercase',
...
function DaycareRestrictedView({ t, accent }) {
  ...
  return (
    <div style={{
      ...
      padding: 'var(--k-pad)', gap: 'var(--k-gap)',
      flex: 1,
    }}>
      <div style={{
        padding: 'var(--k-pad)',
        ...
        <div className="k-display" style={{
          fontSize: 24, lineHeight: 1.15, marginTop: 8, fontWeight: 500,
        }}>{t.childName}</div>
        <div style={{
          fontSize: 13, color: 'var(--k-ink-2)', marginTop: 4,
        }}>{given ? t.daycareDone : t.daycarePrompt}</div>
      </div>
      ...
      <button
        ...
        style={{
          ...
          fontFamily: 'inherit',
          minHeight: 96, cursor: given ? 'default' : 'pointer',
          transition: 'all 0.2s',
        }}
      >
        <div style={{
      ...
      <div style={{
        fontSize: 11, color: 'var(--k-ink-4)', textAlign: 'center',
        fontStyle: 'italic',
      }}>{t.notMedicalDevice}</div>
    </div>
  );
}
...
function KinhaleHome({
  lang = 'fr', density = 'regular', type = 'grotesk',
  role = 'admin', time = 'evening',
  layout = 'cards', dark = false, accent,
}) {
  const t = COPY[lang];
    ...
    >
      <div style={{ flex: 1, minHeight: 0, overflow: 'auto' }}>
        ...
        <div style={{
          ...
          display: 'flex', flexDirection: 'column', gap: 'var(--k-gap)',
        }}>
  ...
  return (
    <div style={{
      ...
      <div style={{ minWidth: 0 }}>
        ...
        <div className="k-display" style={{
          fontSize: 28, lineHeight: 1.1, marginTop: 4, fontWeight: 500,
        }}>{t.childName}</div>
      </div>
      <div style={{
        ...
        <span style={{
          width: 6, height: 6, borderRadius: 99, background: 'var(--k-ok)',
          boxShadow: '0 0 0 3px var(--k-ok-soft)',
        }} />
        {role === 'admin' ? t.role_admin : role === 'restricted' ? t.role_restricted : t.role_contributor}
      </div>
    </div>
  );
}
